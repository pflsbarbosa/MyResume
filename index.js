...
function myAge(){
  const today = new Date(); //captar a data de hoje
  const myBirthdate =  new Date("12/01/1971");  //meu dia de aniversário
  
 
  // a linha abaixo dá o retorno da idade, baseando-se apenas no ano, //subitraindo do ano atual o ano de nascimento ex: 2021-1990 = 31.  
  let age = today.getFullYear() - myBirthdate.getFullYear();
  // a linha abaixo verifica se estamos no mesmo mês do aniversário
  const monthDiference = today.getMonth() - myBirthdate.getMonth();
  
 if (monthDiference < 0 || monthDiference == 0 && today.getDate() < myBirthdate.getDate()){
   age = age -1; // se não for o mês e a data exata diminui 1
 }
 
  return document.getElementById("myAge").innerHTML =  age;
}
